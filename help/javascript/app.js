...
tiles.addTo(map)
 ...
 citiesMarker = new L.FeatureGroup();
...
function getCluster() {
  $.ajax({
    ...
    success: function(response) {
      ...
      let south = response['info'][0]['south'];
      let north = response['info'][0]['north'];
      let west = response['info'][0]['west'];
      let east = response['info'][0]['east'];
